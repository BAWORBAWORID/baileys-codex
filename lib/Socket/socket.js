...
export const makeSocket = (config) => {
    ...
    const checkAccountRestriction = async () => {
        try {
            const result = await query({
                ...
                attrs: {
                    to: S_WHATSAPP_NET,
                    type: 'get',
                    xmlns: 'urn:xmpp:whatsapp:account_restriction'
                }
            });
            const restrictionNode = result?.content?.[0];
            if (restrictionNode && restrictionNode.tag === 'restricted') {
                return {
                    isRestricted: true,
                    reachoutTimelock: restrictionNode.attrs['reachout_timelock'] || '0',
                    messageCap: restrictionNode.attrs['message_cap'] || '0',
                    reason: restrictionNode.attrs['reason'] || 'unknown'
                };
            }
            return {
                isRestricted: false,
            ...
            return { isRestricted: false, error: error.message };
        }
    };
